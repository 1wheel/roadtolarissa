...
I was a little slow putting everything together; 538 put out a [similar piece](http://fivethirtyeight.com/features/republicans-voter-registration-gains-probably-arent-gains-at-all/) yesterday  
...
Dorothy Gambrell made a full page spread for Businessweek showing the most common marriages between different occupations. The print version was a little too large and dense for the web, so I started with a clean slate design wise. I initially made a bunch of different scatter plots looking at income, sex ratio, same sex relationships, and the distribution of other interesting slices of data. The data ended up being a little to much to do a thorough analysis and write up of on a short deadline, so I decided to go with a more of a data-art approach.

A bit frustrated my inability to come up with an interesting way of grouping or arranging the jobs while still fitting all 500+ jobs on the screen, I laid them all out as grid to get a sense the minimal density I needed to accommodate. Once the type was small enough to see all the jobs at once, connecting the most common pairings with lines (just like print version) sort of suggested itself. I don't like how the graphic requires interaction - there are just too many connections show any sort of high level summary - but since it is aiming to be a fun valentines day piece, I can live with that. I would have also liked to show the most distinctive pairings, not just the most common.
