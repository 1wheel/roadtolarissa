...
The final version gets better performance by passing in [array of attributes](https://bl.ocks.org/1wheel/9b3bcc4ce8266913c0a0ddd4120a41de) to the vertex shader - after publishing the very helpful [regl chat](http://gitter.im/mikolalysenko/regl) showed me how to fix.
...
With the data file getting bigger and bigger during the storm, the Post reduced the temporal resoultion of the their animation. I didn’t. Keeping the data hourly really shows the movement of the storm. And if loading a 10 MB file on the page was a bad idea, why is there 50 MB video ad a little farther down? (On mobile, where bandwidth matters more, I decreased the spatial resolution).
...
Will sticking the rainfall map at the bottom, I noticed I was having a hard time [parsing](https://imgur.com/0zHzrwl) height of the bars when they stacked on top of each other. I added a [gradient and a little curve](https://imgur.com/KAK41t0) so the color and angle of the curve would also encode the flooding height. Also they looked kind of like rain or water drops!

I’m not sure why we stuck with the bars; it got rushed out to publication before we got a chance to hash it out.   
...
The timeseries and call stream animation assumed they recived the calls in order of start time; when they started showing on the front end unordered they caused lots hard to track down bugs. I thought there was an error with our data processing, but by [charting](https://imgur.com/NWM3UoP) the call center timestamp against the upload time found that high call volumes led to a [bottle neck](https://imgur.com/GIXN8eC). We ended up randomly distributing the new calls we recived over the next 15 minutes—not as magically as seeing them show up as they happened or exactly replayed, but the best we could do.

 I didn’t end up watching too many polls come in as they happend. I really wanted to make the stream animation a little slicker and less laggy, but got [bogged down](https://twitter.com/Rich_Harris/status/1037903141960867840) coming up with awful hacks to cram our sleek 100kb svelte app inside of the NYT’s react app. Kids, always make sure everyone is on board with your planned publishing pipeline *before* you start working.
...
I tried lots of ideas for showing the margin of uncertininty for individual house races. A [lavalamp](https://bl.ocks.org/1wheel/raw/37344ee99e7d23dee2122bdd08c84d92/), a [bouncing histogram](https://bl.ocks.org/1wheel/raw/c806ed704adee9ec82e580dff7763388/), a grid of needles. 



https://bl.ocks.org/1wheel/raw/c8d5a69a194de0ff69192f7951b3d514/

https://bl.ocks.org/1wheel/raw/692e7bb028b822a74a5517daf8d946c5/
...
Bui build similar charts for governors and overall state control, then dropped the [baseline](https://imgur.com/NnIwauh) to make them denser. I thought the shape of the outlines looked nice, but with three catorgies it is totally unnessary. After publishing, Kevin pointed the Times had previously published [both](https://imgur.com/HSLBNSx) [types](https://imgur.com/8joPL2f).
...
I knew this would be the last breaking news thing I’d work on so I tried to get the little touches right, sticking in [hover+opacity](https://www.nytimes.com/interactive/2016/upshot/presidential-polls-forecast.html---https://imgur.com/NmGA7MO) effect lifted from Gregor. 
